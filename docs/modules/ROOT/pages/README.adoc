...
https://travis-ci.com/saltstack-formulas/consul-formula[image:https://travis-ci.com/saltstack-formulas/consul-formula.svg?branch=master[Travis CI Build Status]]
...
Adds the Consul service startup configuration or script to an operating
system.

To start a service during Salt run and enable it at boot time, you need
to set following Pillar:
...
Creates the docker instance and runs the `consul` main state, ready for
testing.
